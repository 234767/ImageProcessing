use crate::*;
use image_proc::modifications::filters::basic::gpu::GMeanFilterGPU;

fn test_gmean_filter_gpu(width: u32, height: u32, channel: usize) {
    let mut image = sample_image();

    let filter = GMeanFilterGPU::try_new(width, height).expect("Failed to create filter");
...
invoke_test! {test_gmean_filter_gpu {
    gmean_3x3_red  (3,3,0),
