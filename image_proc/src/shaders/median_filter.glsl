...
uint num_elements;
uint luminosity_buckets[256];

void clear_values() {
    num_elements = 0;
    for (int i = 0; i < 256; i++) {
        luminosity_buckets[i] = 0;
    }
}

uint get_median() {
    uint median_index = num_elements / 2;
    uint current_index = 0;
    for (int luminosity = 0; luminosity < 256; luminosity++) {
        current_index += luminosity_buckets[luminosity];
...
void main() {
    ...
    clear_values();
    COLLECT_PIXELS (
        uint luminosity = float_to_uint(pixel.r);
        luminosity_buckets[luminosity]++;
        num_elements++;
        )
    to_write.r = uint_to_float(get_median());

    clear_values();
    COLLECT_PIXELS (
        uint luminosity = float_to_uint(pixel.g);
        luminosity_buckets[luminosity]++;
        num_elements++;
        )
    to_write.g = uint_to_float(get_median());

    clear_values();
    COLLECT_PIXELS (
        uint luminosity = float_to_uint(pixel.b);
        luminosity_buckets[luminosity]++;
        num_elements++;
        )
    to_write.b = uint_to_float(get_median());
